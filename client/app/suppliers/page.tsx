...
// ✅ FIX: Spring hata ke tween — fast & predictable on mobile
const modalVariants: Variants = {
...
export default function SuppliersPage() {
  ...
  // ✅ FIX: setShowModal sabse last mein — state ek saath set ho
  const openAdd = () => {
    setForm(initForm);
    setEditSupplier(null);
    setShowModal(true);
  ...
  return (
    <AppLayout>
      ...
      <AnimatePresence>
        {showModal && (
          <>
            {/* ✅ FIX: backdrop-blur-sm hata diya — GPU load kam on mobile */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 pointer-events-none">
              <motion.div
                ...
                animate="visible"
                exit="exit"
                className="bg-white rounded-t-3xl sm:rounded-3xl border border-white shadow-2xl w-full sm:max-w-2xl max-h-[92vh] overflow-y-auto pointer-events-auto flex flex-col"
              >
                {/* Header */}
                <div className="flex items-center justify-between p-6 md:p-8 border-b border-slate-100 bg-slate-50/50 sticky top-0 z-10">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 border border-indigo-100">
                      <Building2 className="w-6 h-6" />
                    </div>
